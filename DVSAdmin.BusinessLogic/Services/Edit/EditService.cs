    ...
    public class EditService : IEditService
    {
        ...
        private static void GetServiceKeyValueForSchemeMappingsTFV0_4(ServiceDraftDto currentData, ServiceDto previousData, Dictionary<string, List<string>> currentDataDictionary, Dictionary<string, List<string>> previousDataDictionary)
        {
            ...
            if (currentData.ServiceSupSchemeMappingDraft.Count>0 && currentData.ServiceSupSchemeMappingDraft.Count < previousData?.ServiceSupSchemeMapping?.Count)
            {
                ...
                foreach (var schemeMapping in previousData.ServiceSupSchemeMapping)
                {
                    ...
                    if(previousSchemeMapping != null)
                    {
                        ...
                        if (currentMappingDraft == null)//removed  scheme
                        {
                            ...
                            previousSchemeGpg44ProtectionMappings != null && previousSchemeGpg44ProtectionMappings.Count > 0 ? previousSchemeGpg44ProtectionMappings : [@Constants.NullFieldsDisplay]);

                            previousDataDictionary.Add(schemeMapping.SupplementaryScheme.SchemeName + ":" + Constants.GPG44Authentication,
                            previousSchemeGpg44AuthenticationMappings != null && previousSchemeGpg44AuthenticationMappings.Count > 0 ? previousSchemeGpg44AuthenticationMappings : [@Constants.NullFieldsDisplay]);
                ...
                List<int> previousSchemeIds = previousData.ServiceSupSchemeMapping.Select(x => x.SupplementarySchemeId).ToList();
                // Find items in currentData that are not in previousData
                var newItems = currentData.ServiceSupSchemeMappingDraft.Where(item => !previousSchemeIds.Contains(item.SupplementarySchemeId)).ToList();

                foreach (var schemeMappingDraft in newItems)
                {
                    MapSchemeMappings(previousData, currentDataDictionary, previousDataDictionary, schemeMappingDraft);
                }
        ...
        private static void MapSchemeMappings(ServiceDto previousData, Dictionary<string, List<string>> currentDataDictionary, Dictionary<string, List<string>> previousDataDictionary, ServiceSupSchemeMappingDraftDto? schemeMappingDraft)
        {
